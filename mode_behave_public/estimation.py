...
class Estimation:
    ...
    def loglike_MXL(self, **kwargs):
        ...
        ML_log = np.log(self.calculate_mixed_logit(points_in=points_in))
        return np.sum(ML_log[np.isfinite(ML_log)])
    
    def calculate_mixed_logit(self, **kwargs):
        """
        ...
        try:
            res = np.sum(np.multiply(self.shares,self.logit_probs.T).T, axis=0)
        except:       
            ...
            @guvectorize(
                ...
                '(m,p),(n,l),(i,j,n,l)->(m,l)', nopython=True, target="parallel"
                )
            def calculate_logit_vector(points, av, data, logit_probs_):
                ...
                """    
                #logit_probs_ = np.zeros(shape = (points.shape[0], av.shape[1]))
                    ...
                    for l in prange(av.shape[1]):
                        ...
                        for c in prange(count_c):   
                            ...
                            top += av[c][l] * choice[c][l] * exp(
                                get_utility_vector(c, point, l, data)
                                )
                                            
                        logit_probs_[m][l] = top/bottom  
                
            #return logit_probs_
            self.points = points_in
            self.logit_probs = calculate_logit_vector(self.points, self.av, data)
            res = np.sum(np.multiply(self.shares.values,self.logit_probs.T), axis=1)
